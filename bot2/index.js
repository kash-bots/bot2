...
const apivhtear = 'MR******' //HAHAHA
const ZeksApi = 'apivinz'
const BarBarKey = 'Q7MLdUnV**********' //HAHAHA
const shizukaapi = 'onlyonedeveloper'
const TobzApi = 'BotWeA'
const XteamKey = 'BELI SENDIRI:)'
...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=62814622392081:+6284622392081\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
const client = new WAConnection()
   ...
   console.log(color('(+)','white'),color('Mr.A43G','red'),color('(+)','white'),color('SQAN CODENYA!!','blue'),color('YOU','white'),color('TUBE','red'),color('Mr.A43G','yellow'))
})
...
break
		...
		if (!isRegistered) return reply( ind.noregis())
			...
			case 'toxic':
                ...
                client.sendMessage(from, buf, image, {quoted: mek, caption: "DONE BOS✓"})})
                break
				case 'attp':
                 if (!isRegistered) return reply( ind.noregis())
					if (args.length < 1) return reply(ind.wrongf())
			...
			case 'animefanart':				//UPDATE
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/random/animefanart?apikey=${apiKey}`, {method: 'get'})
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
					case 'megumin':				//UPDATE
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					case 'bj':				//UPDATE
                 if (!isRegistered) return reply( ind.noregis())
					buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/bj?apikey=${apiKey}`, {method: 'get'})
					client.sendMessage(from, buffer, image, {quoted: mek })
					break
					case 'neko3':				//UPDATE
                 if (!isRegistered) return reply( ind.noregis())
		...
		if (!isRegistered) return reply(ind.noregis())
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/kusonimesearch?apikey=${apiKey}`, {method: 'get'})
	...
	case 'hobby':
		...
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('[RIUBOT]','yellow'), ('Command Tidak Ditemukan!!','red'), color(sender.split('@')[0]))
					}
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
